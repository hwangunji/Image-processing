...
int main() {
	...
	//이미지 로드
	img_gray = imread("C:\\Users\\happy\\OneDrive\\바탕 화면\\!막학기최선다해\\영상처리\\화소값 기반 처리 Code\\4.화소값기반처리\\logo.jpg", IMREAD_GRAYSCALE);
	...
	//connectedComponentsWithStats라벨링 지원하는 함수 레이블링할 이미지,결과이미지,
	//이미지배열 통계,레이블링된이미지 중심좌표,8연결성,결과이미지 데이터 타입
